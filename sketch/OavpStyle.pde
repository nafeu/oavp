public class OavpStyle {
  ...
  OavpStyle(int colorSeed) {
    this.colorSeed = colorSeed;
    flat = new FlatUIColors();
  }

  void setColorSeed(int colorSeed) {
    this.colorSeed = colorSeed;
  }
  ...
  int getRandomColor(int x, int y) {
    int a = floor((x * colorSeed) - (y * y)) + x;
    int b = floor((y * colorSeed) - (x * x)) + y;
    return material[(colorSeed + abs(a + b)) % material.length];
  }
